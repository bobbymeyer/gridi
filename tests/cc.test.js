...
test('a CC node still passes the pulse on', () => {
  const { p } = ccPatch();
  const { notes, controls } = run(p, 2);
  assert.ok(notes.length >= 3, 'the note after it still plays');
  assert.equal(notes.length, controls.length, 'one of each per pulse');
});

test('values are held to seven bits whatever the node says', () => {
  const { p } = ccPatch({ mode: 'sequence', values: '-40 200 63.7' });
  const { controls } = run(p, 2);
...
test('random and walk stay inside the range asked for', () => {
  for (const mode of ['random', 'walk']) {
    const { p } = ccPatch({ mode, min: 20, max: 40, amount: 5 });
    const { controls } = run(p, 4);
...
test('node and signal scopes are untouched by any of this', () => {
  ...
  assert.deepEqual(notes.slice(0, 3).map((n) => n.note), [60, 64, 67]);
});

...
| GeneralUser GS ships in the repository, and loads on first run | Asking everyone to find a SoundFont before anything makes a sound | Thirty-two megabytes in a repository is a real cost, paid once against every visitor who would otherwise hear a triangle wave. Its licence allows redistribution and asks that nobody hotlink the author's files, so a local copy is what he wants |
| A zone naming one half of a stereo pair plays both, through the link | Playing the half that was named, as it did before | Half a stereo recording is a recording with a hole in it. A font that pans both sides itself already lists both, so nothing is added there; only a pair assembled here is positioned here |
| Modulators read velocity and key, and nothing else | Every source the spec names | Gridi has no wheel, no pedals, no aftertouch and no bender. A modulator reading one of those would multiply by a number nobody set |
| Velocity is applied by the font's routings, not by the player | A velocity curve in the player, as it was before modulators | Doing both counts velocity twice: half velocity landed near a quarter of the amplitude instead of a half. The font knows what its own instruments should do with it |
